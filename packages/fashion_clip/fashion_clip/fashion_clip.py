...
from .utils import get_cache_directory, _is_hugging_face_repo, _model_processor_hash
from .utils import _download, file_sha256, display_images_from_s3, display_images_from_url, display_images
...
class FashionCLIP:
    ...
    def encode_image_tensors(self, images):
        new_batch = {}
        new_batch['pixel_values'] = (images - self.clip_mean[None, :, None, None]) / self.clip_std[None, :, None, None]
        image_embeddings = self.model.get_image_features(**new_batch)
        return image_embeddings

    def encode_text_tensors(self, text):
        ...
        for batch in dataloader:
            batch = {k: v.to(self.device) for k, v in batch.items()}
        text_embeddings = self.model.get_text_features(**batch)
    ...
    def display_attention(self, image_path, query_text, pixel_size=15, iterations=5):
        ...
        display_heatmap(image, query_text, heatmap)
